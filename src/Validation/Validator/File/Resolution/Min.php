...
{
    const TYPE = 'FileResolutionMin';

    const OPT_RESOLUTION = 'resolution';

    const OPT_INCLUDED = 'included';

    protected $message = ':field的图像分辨率不能小于:resolution';
    ...
    public function validate(\Snow\Validation\Validation $validation, $field)
    {
        ...
        if ($tmp) {
            ...
            $result = $included ? ($width >= $minWidth && $height >= $minHeight) : ($width > $minWidth && $height > $minHeight);
            if ($result) {
                return true;
            }
        }
